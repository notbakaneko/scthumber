 ...
 * @param {Buffer} inputBuffer
 */
function optimizeImage(inputBuffer) {
  return sharp(inputBuffer)
    .jpeg({'quality': 100})
    .on('error', console.error) // needed before the pipe to mozjpeg to prevent death.
    .pipe(
      mozjpeg({'quality': 94, 'args': '-baseline'})
    ).on('error', console.error); // every pipe needs it's own handler.
}
...
exports = module.exports = function (opts) {
  ...
  async function thumbnail(req, res) {
    ...
    try {
      ...
      optimizeToResponse(image, res, req)
  ...
  async function optimize(req, res) {
    ...
    try {
      ...
      // FIXME: need to not finish on stream error
      optimizeToResponse(data, res, req)
        .on('finish', () => logOk(req, data, request_start));
    } catch (err) {
      handleFetchError(err, req, res);
    }
   ...
   * @param {Date} startTime
   */
  function optimizeToResponse(buffer, res, req) {
    return optimizeImage(buffer)
      .on('error', (error) => {
        log(`ERR: ${req.originalUrl} [${error}]`.red);
        errorAbort(res, `Error resizing image: ${error}`, 'thumb_error', 500);
      })
      .pipe(res);
  }
